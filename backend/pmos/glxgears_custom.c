...
apk add build-base mesa-dev libx11-dev libxext-dev libxi-dev
gcc -o glxgears glxgears.c -lGL -lX11 -lXext -lXi -lglut
...
char displayText[MAX_TEXT_LENGTH] = "Hello, World!";
...
draw(void)
{
    glClearColor(1.0, 1.0, 1.0, 1.0); // Blanc (R,G,B,A)
   ...
   for (i = 1; i < argc; i++) {
      if (strcmp(argv[i], "-display") == 0) {
         ...
         i++;
            ...
            if (i < argc-1) {
                strncpy(displayText, argv[i+1], MAX_TEXT_LENGTH-1);
                displayText[MAX_TEXT_LENGTH-1] = '\0';
                i++;
            } else {
                printf("Error: -text option requires an argument.\n");
                usage();
                return -1;
            }
        }
